...
unpack() {
    ...
    local url="$1"
    local tarball; tarball="$(fetch "$url")"
    local dir; dir="$(extract "$tarball")"
...
build_binutils_pass1() {
    local src; src="$(unpack "$BINUTILS_URL")"
...
build_gcc_pass1() {
    local src; src="$(unpack "$GCC_URL")"
    cd "$src"

    # GCC needs GMP, MPFR, MPC unpacked inside its source tree
    tar -xf "$(fetch "$MPFR_URL")"; mv -v "mpfr-${MPFR_VERSION}" mpfr
    tar -xf "$(fetch "$GMP_URL")";  mv -v "gmp-${GMP_VERSION}"  gmp
    tar -xf "$(fetch "$MPC_URL")";  mv -v "mpc-${MPC_VERSION}"  mpc
...
build_linux_headers() {
    ...
    local tarball; tarball="$(fetch "$kern_url")"
...
build_glibc() {
    local src; src="$(unpack "$GLIBC_URL")"
    cd "$src"
...
build_ncurses() {
    local src; src="$(unpack "$NCURSES_URL")"
    cd "$src"
...
build_coreutils() {
    local src; src="$(unpack "$COREUTILS_URL")"
    with_clean_build "$src"
    _cross_install --prefix=/usr                        \
...
build_diffutils() {
    local src; src="$(unpack "$DIFFUTILS_URL")"
    with_clean_build "$src"
    _cross_install --prefix=/usr
    rm -rf "$src"
}
...
build_findutils() {
    local src; src="$(unpack "$FINDUTILS_URL")"
    with_clean_build "$src"
    _cross_install --prefix=/usr                  \
...
build_patch() {
    local src; src="$(unpack "$PATCH_URL")"
    with_clean_build "$src"
    _cross_install --prefix=/usr
    rm -rf "$src"
}
